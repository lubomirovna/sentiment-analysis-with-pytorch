...
if os.path.exists(parsed_args.pathToModel):
    global PATH_TO_MODEL
    PATH_TO_MODEL = parsed_args.pathToModel
if os.path.exists(parsed_args.inputDirectory):
    global INPUT_DIR
    INPUT_DIR = parsed_args.inputDirectory


train_data, test_data = create_datasets(f'{INPUT_DIR}training_tweets.csv')

tokenizer = torchtext.data.utils.get_tokenizer("basic_english")

# load the vocabulary
vocab = torch.load(f'{INPUT_DIR}vocab.pth')
...
checkpoint = torch.load(f'{PATH_TO_MODEL}model_1d.pth')
...
print(
    f"Previously trained for {folds+1} number of folds and for {epochs+1} number of epochs...")
...
for fold, (train_idx, val_idx) in enumerate(
    ...
    # resume training from the fold it was stopped on
    if fold < folds:
        continue
    ...
    for epoch in range(epochs, N_EPOCHS):
        ...
        # save the best model till now if it is the least loss in the current
        # epoch
        save_best_model(
            valid_epoch_loss, epoch, model, optimizer, criterion, PATH_TO_MODEL
        )

        # save the latest model after epoch is completed
        save_model(epoch, model, optimizer, criterion, fold, PATH_TO_MODEL)
